...
static sgx_status_t SGX_CDECL Enclave_ocall_enter_int(void* pms)
{
	ms_ocall_enter_int_t* ms = SGX_CAST(ms_ocall_enter_int_t*, pms);
	ocall_enter_int(ms->ms_ptr);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_enter_string(void* pms)
{
	ms_ocall_enter_string_t* ms = SGX_CAST(ms_ocall_enter_string_t*, pms);
	ocall_enter_string(ms->ms_ptr);
...
static const struct {
	...
	13,
	{
		...
		(void*)Enclave_ocall_enter_int,
		(void*)Enclave_ocall_enter_string,
...
sgx_status_t fill_mem(sgx_enclave_id_t eid)
{
	sgx_status_t status;
...
sgx_status_t new_student(sgx_enclave_id_t eid, int* retval, int n)
{
	sgx_status_t status;
	ms_new_student_t ms;
	ms.ms_n = n;
...
sgx_status_t new_class(sgx_enclave_id_t eid, int* retval, int n)
{
	sgx_status_t status;
	ms_new_class_t ms;
	ms.ms_n = n;
...
sgx_status_t print_all_students(sgx_enclave_id_t eid)
{
	sgx_status_t status;
...
sgx_status_t public_print_student(sgx_enclave_id_t eid, int* retval, int id)
{
	sgx_status_t status;
	ms_public_print_student_t ms;
	ms.ms_id = id;
...
sgx_status_t print_all_classes(sgx_enclave_id_t eid, int* retval)
{
	sgx_status_t status;
	ms_print_all_classes_t ms;
	status = sgx_ecall(eid, 32, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t public_print_class(sgx_enclave_id_t eid, int* retval, int id)
{
	sgx_status_t status;
	ms_public_print_class_t ms;
	ms.ms_id = id;
